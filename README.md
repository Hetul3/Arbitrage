...
10. `arb_engine` – consumes `matches.live`, simulates both directions with depth-aware fills + Kalshi fees, and logs every result (profitable or not). Configure `ARB_ENGINE_WORKERS`, `ARB_ENGINE_GROUP`, and `ARB_ENGINE_BUDGET_USD` to tune throughput/budget.
