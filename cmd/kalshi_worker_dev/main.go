...
func main() {
	...
	logCache := envBool("REDIS_EMBED_LOG_HITS", true)
...
func mustFinder(client *chroma.Client, collectionID string, debug bool, verdictCache cache.VerdictCache) *matcher.Finder {
	...
	if err != nil {
		log.Fatalf("[kalshi-worker-dev] matcher config: %v", err)
	}
	return finder
}
...
func mustEmbeddingCache() cache.EmbeddingCache {
	...
	if err != nil {
		log.Fatalf("[kalshi-worker-dev] redis cache: %v", err)
	}
	return cacheClient
}
...
func mustVerdictCache() cache.VerdictCache {
	...
	if err != nil {
		log.Fatalf("[kalshi-worker-dev] redis verdict cache: %v", err)
	}
	return cacheClient
}
...
func setupMatchWriter(ctx context.Context, brokers []string) *kafkago.Writer {
	...
	defer cancel()
	if err := kafka.EnsureTopic(ensureCtx, brokers, topic); err != nil {
		log.Printf("[kalshi-worker-dev] ensure matches topic warning: %v", err)
	}
...
func publishMatch(ctx context.Context, writer *kafkago.Writer, source *models.MarketSnapshot, res *matcher.Result) {
	...
	if err != nil {
		log.Printf("[kalshi-worker-dev] marshal match error: %v", err)
		return
	}
	msg := kafkago.Message{
	...
	if err := writer.WriteMessages(ctx, msg); err != nil {
		log.Printf("[kalshi-worker-dev] publish match error: %v", err)
	}
}
