...
func main() {
	...
	processor := workers.NewProcessor(embedClient, chromaClient, collectionID, "kalshi", embedCache, logCache)
	...
	logging.Infof("[kalshi-worker] consuming %s with group %s (%d workers)", topic, group, workerCount)
	workers.Run(ctx, brokers, topic, group, workerCount, func(ctx context.Context, snap *models.MarketSnapshot) error {
		...
		logging.Infof("[kalshi-worker] upserted market=%s event=%s", snap.Market.MarketID, snap.Event.EventID)
		return nil
	})
}
...
func mustEmbedClient() *embed.Client {
	...
	if err != nil {
		logging.Fatalf("[kalshi-worker] embed client: %v", err)
	}
	return client
}
...
func mustChromaClient(ctx context.Context) (*chroma.Client, string) {
	...
	if err != nil {
		logging.Fatalf("[kalshi-worker] ensure chroma collection: %v", err)
	}
...
func mustFinder(client *chroma.Client, collectionID string, debug bool, verdictCache cache.VerdictCache) *matcher.Finder {
	...
	if err != nil {
		logging.Fatalf("[kalshi-worker] matcher config: %v", err)
	}
	return finder
}
...
func setupMatchWriter(ctx context.Context, brokers []string) *kafkago.Writer {
	...
	defer cancel()
	if err := kafka.EnsureTopic(ensureCtx, brokers, topic); err != nil {
		logging.Errorf("[kalshi-worker] ensure matches topic warning: %v", err)
	}
...
func publishMatch(ctx context.Context, writer *kafkago.Writer, source *models.MarketSnapshot, res *matcher.Result) {
	...
	if err != nil {
		logging.Errorf("[kalshi-worker] marshal match error: %v", err)
		return
	}
	msg := kafkago.Message{
	...
	if err := writer.WriteMessages(ctx, msg); err != nil {
		logging.Errorf("[kalshi-worker] publish match error: %v", err)
	}
}
...
func mustEmbeddingCache() cache.EmbeddingCache {
	...
	if err != nil {
		logging.Fatalf("[kalshi-worker] redis cache: %v", err)
	}
	return cacheClient
}
...
func mustVerdictCache() cache.VerdictCache {
	...
	if err != nil {
		logging.Fatalf("[kalshi-worker] redis verdict cache: %v", err)
	}
	return cacheClient
}
